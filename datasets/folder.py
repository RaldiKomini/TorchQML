...
# ---- splitting / sampling ----
def pickdata(root, used, ntr=1000, nval=300, nte=300):
    print("n is ", ntr)
    base_ds = ImageFolder(root=root)
    
    # Original indices of the selected classes in the full dataset
    ...
    # and group paths per original label
    ...
    # For each used class: split and assign new labels 0,1,...
    for old_lab in used_idx:
        new_lab = old_to_new[old_lab]

        paths_tr, paths_va, paths_te = split(class_div[old_lab], ntr, nval, nte)

        train_samples += [(p, new_lab) for p in paths_tr]
        val_samples   += [(p, new_lab) for p in paths_va]
        test_samples  += [(p, new_lab) for p in paths_te]
...
def image_to_features(img):
    ...
    # 4) Concatenate everything into one feature vector
